    ...
    <div class="test-page-content">
        ...
        <div class="test-page-list">
            <div v-for="(item,index) in test" :key="index" class="test-page-item">
                ...
                <span>
                  <el-button v-if="item.schedule == '未开始'" @click="toTestInfo(item.id, item.schedule)" icon="el-icon-edit" size="mini" type="success">开始考试</el-button>
                  <el-button v-if="item.schedule == '已完成'" @click="toTestInfo(item.id, item.schedule)" icon="el-icon-edit" size="mini" type="success">查看详情</el-button>
